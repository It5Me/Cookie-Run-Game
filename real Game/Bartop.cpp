...
Bartop::Bartop()
{
	...
	this->texture_blood.loadFromFile("Texture/bar/blood.png");
	...
	this->spri_blood.setTexture(this->texture_blood);
	this->spri_heart.setTexture(this->texture_heart);
	this->rect = IntRect(0, 0, this->texture_blood.getSize().x, this->texture_blood.getSize().y);
...
{	
	...
	if (hp >= 0 && hp <= 100) {
		this->rect.width = float(hp / 100.0f) * this->texture_blood.getSize().x;
